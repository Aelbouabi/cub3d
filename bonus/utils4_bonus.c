...
/*   Updated: 2024/11/16 16:33:49 by aelbouab         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../cub3D.h"

void	ver_point1_bonus(t_all *all, float xend, float yend, float xstep)
{
	float	ystep;
...
void	ver_point_bonus(t_all *all, float angle)
{
	...
	ver_point1_bonus(all, xend, yend, xstep);
}
...
void	draw_rm_bonus(t_all *all, float wall_h)
{
	...
	while (i < wall_h / 2)
	{
		mlx_put_pixel(all->cub->img, all->x, y, ft_color_bonus(0, 0, 255, 255));
		y--;
		i++;
	}
	y = all->cub->height / 2;
